...
func TestTaskBlake3020Boundary(t *testing.T){
 d:=blake3.New().Digest();_,e:=d.Seek(5,-7);got:=e!=nil;want:=true;err:=error(nil);if !reflect.DeepEqual(got,want) || err!=nil{t.Fatalf("got=%v want=%v err=%v",got,want,err)}
}
