...
func TestTaskBlake3019Boundary(t *testing.T){
 data:=[]byte{};got:=blake3.Sum512(data);want:=make([]byte,64);blake3.New().Digest().Read(want);gotv:=bytes.Equal(got[:],want);wantv:=true;err:=error(nil);if !reflect.DeepEqual(gotv,wantv) || err!=nil{t.Fatalf("got=%v want=%v err=%v",gotv,wantv,err)}
}
